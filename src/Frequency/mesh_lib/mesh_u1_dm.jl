...
function get_mesh_u1_dm(
    ...
    m = Mesh_u1_dm(num_σ + 1, 
                   ...
                   SVector(ntuple(comp -> get_mesh(10.0 * initial, 750.0 * max(initial, 0.5), num_Ω, p_Ω), 6)), 
                   SVector(ntuple(comp -> get_mesh(10.0 * initial, 500.0 * max(initial, 0.5), num_ν, p_ν), 6)), 
                   SVector(ntuple(comp -> get_mesh(10.0 * initial, 750.0 * max(initial, 0.5), num_Ω, p_Ω), 6)), 
                   SVector(ntuple(comp -> get_mesh(10.0 * initial, 500.0 * max(initial, 0.5), num_ν, p_ν), 6)),
                   SVector(ntuple(comp -> get_mesh(10.0 * initial, 750.0 * max(initial, 0.5), num_Ω, p_Ω), 6)), 
                   SVector(ntuple(comp -> get_mesh(10.0 * initial, 500.0 * max(initial, 0.5), num_ν, p_ν), 6)))
