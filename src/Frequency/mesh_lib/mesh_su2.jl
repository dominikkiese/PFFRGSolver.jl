...
function get_mesh_su2(
    ...
    m = Mesh_su2(num_σ + 1, 
                 num_Ω + 1, 
                 num_ν + 1, 
                 get_mesh(5.0 * initial, 1000.0 * initial, num_σ, p_σ), 
                 SVector(ntuple(comp -> get_mesh(5.0 * initial,  75.0 * initial, num_Ω, p_Ω), 2)), 
                 SVector(ntuple(comp -> get_mesh(5.0 * initial, 150.0 * initial, num_ν, p_ν), 2)), 
                 SVector(ntuple(comp -> get_mesh(5.0 * initial,  75.0 * initial, num_Ω, p_Ω), 2)), 
                 SVector(ntuple(comp -> get_mesh(5.0 * initial, 150.0 * initial, num_ν, p_ν), 2)),
                 SVector(ntuple(comp -> get_mesh(5.0 * initial,  75.0 * initial, num_Ω, p_Ω), 2)), 
                 SVector(ntuple(comp -> get_mesh(5.0 * initial, 150.0 * initial, num_ν, p_ν), 2)))
