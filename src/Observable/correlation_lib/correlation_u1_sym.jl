...
function inner_kernel_xx(
    ...
    bs1 = get_u1_sym_buffer_s(1, v + vp, 0.5 * (v - vp), 0.5 * (-v + vp), m)
    bt1 = get_u1_sym_buffer_t(1, 0.0, v, vp, m)
    bu1 = get_u1_sym_buffer_u(1, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)

    # get buffers for local term
    bs2_2 = get_u1_sym_buffer_s(2, v + vp, 0.5 * (-v + vp), 0.5 * (-v + vp), m)
    bt2_2 = get_u1_sym_buffer_t(2, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)
    bu2_2 = get_u1_sym_buffer_u(2, 0.0, vp, v, m)

    bs2_4 = get_u1_sym_buffer_s(4, v + vp, 0.5 * (-v + vp), 0.5 * (-v + vp), m)
    bt2_4 = get_u1_sym_buffer_t(4, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)
    bu2_4 = get_u1_sym_buffer_u(4, 0.0, vp, v, m)
...
function inner_kernel_zz(
    ...
    # get buffers for non-local term
    bs1 = get_u1_sym_buffer_s(2, v + vp, 0.5 * (v - vp), 0.5 * (-v + vp), m)
    bt1 = get_u1_sym_buffer_t(2, 0.0, v, vp, m)
    bu1 = get_u1_sym_buffer_u(2, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)

    # get buffers for local term
    bs2_1 = get_u1_sym_buffer_s(1, v + vp, 0.5 * (-v + vp), 0.5 * (-v + vp), m)
    bt2_1 = get_u1_sym_buffer_t(1, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)
    bu2_1 = get_u1_sym_buffer_u(1, 0.0, vp, v, m)
...
function inner_kernel_xy(
    ...
    # get buffers for non-local term
    bs1 = get_u1_sym_buffer_s(3, v + vp, 0.5 * (v - vp), 0.5 * (-v + vp), m)
    bt1 = get_u1_sym_buffer_t(3, 0.0, v, vp, m)
    bu1 = get_u1_sym_buffer_u(3, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)

    # get buffers for local term
    bs2_5 = get_u1_sym_buffer_s(5, v + vp, 0.5 * (-v + vp), 0.5 * (-v + vp), m)
    bt2_5 = get_u1_sym_buffer_t(5, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)
    bu2_5 = get_u1_sym_buffer_u(5, 0.0, vp, v, m)

    bs2_6 = get_u1_sym_buffer_s(6, v + vp, 0.5 * (-v + vp), 0.5 * (-v + vp), m)
    bt2_6 = get_u1_sym_buffer_t(6, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m)
    bu2_6 = get_u1_sym_buffer_u(6, 0.0, vp, v, m)
