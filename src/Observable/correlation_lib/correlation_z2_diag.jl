# generate correlation dummy for diagonal z2 symmetry
function get_χ_z2_diag_empty(
    r :: Reduced_lattice,
    m :: Mesh
...
function compute_χ_kernel_xx!(
    ...
    for i in eachindex(buff)
        ...
        # compute value
        buff[i] = -get_Γ_comp(1, site, bs1, bt1, bu1, r, a, apply_flags_z2_diag) / (2.0 * pi)^2

        if site == 1
            vxx, vyy, vzz, vdd  = get_Γ(site, bs2, bt2, bu2, r, a)
            buff[i]            -= (-vxx + vyy - vdd + vzz) / (2.0 * (2.0 * pi)^2)
        end
...
function compute_χ_kernel_yy!(
    ...
    for i in eachindex(buff)
        ...
        # compute value
        buff[i] = -get_Γ_comp(2, site, bs1, bt1, bu1, r, a, apply_flags_z2_diag) / (2.0 * pi)^2
...
function compute_χ_kernel_zz!(
    ...
    for i in eachindex(buff)
        ...
        # compute value
        buff[i] = -get_Γ_comp(3, site, bs1, bt1, bu1, r, a, apply_flags_z2_diag) / (2.0 * pi)^2
