...
function compute_χ(
    ...
    @sync for i in eachindex(r.sites)
        Threads.@spawn begin
            ...
            χ_xx[i]   = quadgk(integrand, -Inf, -2.0 * Λ, 0.0, 2.0 * Λ, Inf, atol = χ_tol[1], rtol = χ_tol[2])[1]

            # compute zz correlation
            integrand = v -> outer_kernel_zz(Λ, i, v, r, m, a, χ_tol)
            χ_zz[i]   = quadgk(integrand, -Inf, -2.0 * Λ, 0.0, 2.0 * Λ, Inf, atol = χ_tol[1], rtol = χ_tol[2])[1]

            # compute xy correlation
            integrand = v -> outer_kernel_xy(Λ, i, v, r, m, a, χ_tol)
            χ_xy[i]   = quadgk(integrand, -Inf, -2.0 * Λ, 0.0, 2.0 * Λ, Inf, atol = χ_tol[1], rtol = χ_tol[2])[1]
        end
    end
