...
function inner_kernel(
    ...
    bs2 = ntuple(comp -> get_buffer_s(comp, v + vp, 0.5 * (-v + vp), 0.5 * (-v + vp), m), 2)
    bt2 = ntuple(comp -> get_buffer_t(comp, v - vp, 0.5 * (v + vp), 0.5 * (v + vp), m), 2)
    bu2 = ntuple(comp -> get_buffer_u(comp, 0.0, vp, v, m), 2)
