function get_unitcell_hyperkagome() :: unitcell
    ...
    basis[2]  = Float64[-1.0 / sqrt(2.0), 1.0 / sqrt(2.0), 0.0] 
    basis[3]  = Float64[ 0.0, 1.0 / sqrt(2.0), 1.0 / sqrt(2.0)]
    basis[4]  = Float64[ 0.0, 2.0 / sqrt(2.0), 2.0 / sqrt(2.0)]
    basis[5]  = Float64[-1.0 / sqrt(2.0), 2.0 / sqrt(2.0), 3.0 / sqrt(2.0)]
    basis[6]  = Float64[-2.0 / sqrt(2.0), 1.0 / sqrt(2.0), 3.0 / sqrt(2.0)]
    basis[7]  = Float64[-2.0 / sqrt(2.0), 0.0,  2.0 / sqrt(2.0)]
    basis[8]  = Float64[-3.0 / sqrt(2.0), 0.0,  3.0 / sqrt(2.0)]
    basis[9]  = Float64[-1.0 / sqrt(2.0), 3.0 / sqrt(2.0), 2.0 / sqrt(2.0)]
    basis[10] = Float64[-2.0 / sqrt(2.0), 3.0 / sqrt(2.0), 1.0 / sqrt(2.0)]
    basis[11] = Float64[-3.0 / sqrt(2.0), 3.0 / sqrt(2.0), 0.0]
    basis[12] = Float64[-3.0 / sqrt(2.0), 2.0 / sqrt(2.0), 1.0 / sqrt(2.0)]

    # define list of Bravais vectors
    vectors    = Vector{Vector{Float64}}(undef, 3)
    vectors[1] = Float64[2.0 * sqrt(2.0), 0.0, 0.0]
    vectors[2] = Float64[0.0, 2.0 * sqrt(2.0), 0.0]
    vectors[3] = Float64[0.0, 0.0, 2.0 * sqrt(2.0)]
