...
"""
function init_model_pyrochlore_global_breathing!(
    ...
    @assert length(J[1]) == 8 "each interaction has to be specified." 
    ...
    for i in eachindex(l.sites)
        ...
        for j in nbs 
            ...
            # add breathing anisotropy
            if (l.sites[j].int - l.sites[i].int)[1 : 3] == [0, 0, 0] 
                J1, J2, J3, J4 = J[1][1:4] # couplings inside unit-cell
            else
                J1, J2, J3, J4 = J[1][5:8] # couplings outside unit-cell
            end
