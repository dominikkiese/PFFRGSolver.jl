...
"""
function init_model_mapleleaf_nn_spin_nematic!(
    J :: Vector{Vector{Float64}},
    l :: Lattice
    ) :: Nothing
    ...
    @assert length(J[1]) == 6 "Need J1, J2, J3, δ1, δ2, δ3 (kl, km and lm nearest-neighbor bonds, and corresponding XXZ anisotropy)"
    
    J1, J2, J3, δ1, δ2, δ3 = J[1]
    ...
    for i in eachindex(l.sites)
        ...
        for j in nbs1
            ...
            if abs.(ints[1]-ints[2]) in [1, 5]
                add_bond!(J1 - δ1, l.bonds[i, j], 1, 1)
                add_bond!(J1 - δ1, l.bonds[i, j], 2, 2)
                add_bond!(J1 + δ1, l.bonds[i, j], 3, 3)
            #km-coupling
            elseif ints in [(1, 5), (5, 1), (3, 5), (5, 3), (1, 3), (3, 1),
                            (2, 6), (6, 2), (2, 4), (4, 2), (4, 6), (6, 4)]
                add_bond!(J2 - δ2, l.bonds[i, j], 1, 1)
                add_bond!(J2 - δ2, l.bonds[i, j], 2, 2)
                add_bond!(J2 + δ2, l.bonds[i, j], 3, 3)
            #lm coupling (dimer)
            else
                add_bond!(J3 - δ3, l.bonds[i, j], 1, 1)
                add_bond!(J3 - δ3, l.bonds[i, j], 2, 2)
                add_bond!(J3 + δ3, l.bonds[i, j], 3, 3)
            end
        end
    end    
    return nothing
