...
function compute_channel_s_kat!(
    ...
    for i in eachindex(da.Γ)
        if kernel == 1
            da.Γ[i].ch_s.q1[:, w1] .= view(tbuff[1], i, :)
        elseif kernel == 2
            da.Γ[i].ch_s.q2_1[:, w1, w2] .= view(tbuff[1], i, :)
        elseif kernel == 3
            da.Γ[i].ch_s.q2_2[:, w1, w3] .= view(tbuff[1], i, :)
        else
...
function compute_channel_s_left!(
    ...
    for i in eachindex(da_l.Γ)
        if kernel == 1
            da_l.Γ[i].ch_s.q1[:, w1] .= view(tbuff[1], i, :)
        elseif kernel == 2
            da_l.Γ[i].ch_s.q2_1[:, w1, w2] .= view(tbuff[1], i, :)
        elseif kernel == 3
            da_l.Γ[i].ch_s.q2_2[:, w1, w3] .= view(tbuff[1], i, :)
        else
...
function compute_channel_s_central!(
    ...
    for i in eachindex(da_c.Γ)
        if kernel == 1
            da_c.Γ[i].ch_s.q1[:, w1] .= view(tbuff[1], i, :)
        elseif kernel == 2
            da_c.Γ[i].ch_s.q2_1[:, w1, w2] .= view(tbuff[1], i, :)
        elseif kernel == 3
            da_c.Γ[i].ch_s.q2_2[:, w1, w3] .= view(tbuff[1], i, :)
        else
