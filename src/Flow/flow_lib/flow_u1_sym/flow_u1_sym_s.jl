...
function compute_s_kat!(
    ...
    bs1 = ntuple(comp -> get_u1_sym_buffer_s(comp, s, vs, -v, m), 6)
    bt1 = ntuple(comp -> get_u1_sym_buffer_t(comp, v - vs, 0.5 * (s + v + vs), 0.5 * (s - v - vs), m), 6)
    bu1 = ntuple(comp -> get_u1_sym_buffer_u(comp, v + vs, 0.5 * (s - v + vs), 0.5 * (s + v - vs), m), 6)

    # get buffers for right vertex
    bs2 = ntuple(comp -> get_u1_sym_buffer_s(comp, s, v, vsp, m), 6)
    bt2 = ntuple(comp -> get_u1_sym_buffer_t(comp, -v - vsp, 0.5 * (s + v - vsp), 0.5 * (s - v + vsp), m), 6)
    bu2 = ntuple(comp -> get_u1_sym_buffer_u(comp, v - vsp, 0.5 * (s + v + vsp), 0.5 * (s - v - vsp), m), 6)
