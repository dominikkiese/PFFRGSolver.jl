...
function compute_dΣ!(
    ...
    @sync for i in 2 : length(m.σ)
        Threads.@spawn begin
            ...
            da.Σ[i]   = quadgk(integrand, -Inf, -2.0 * Λ, 2.0 * Λ, Inf, atol = 1e-10, rtol = 1e-3, maxevals = 10^4)[1]
        end
    end
...
function compute_dΣ_corr!(
    ...
    @sync for i in 2 : length(m.σ)
        Threads.@spawn begin
            ...
            da_Σ.Σ[i] = quadgk(integrand, -Inf, Inf, atol = 1e-10, rtol = 1e-3, maxevals = 10^4)[1]
        end
    end
    ...
    @sync for i in 2 : length(m.σ)
        Threads.@spawn begin
            ...
            da.Σ[i]   += da_Σ.Σ[i]
            da.Σ[i]   += quadgk(integrand, -Inf, Inf, atol = 1e-10, rtol = 1e-3, maxevals = 10^4)[1]
        end
    end
