...
function compute_dΣ_kernel(
    ...
    val = get_Γ_comp(1, 1, b2s, b2t, b2u, r, a, apply_flags_z2_diag) + 
          get_Γ_comp(2, 1, b2s, b2t, b2u, r, a, apply_flags_z2_diag) + 
          get_Γ_comp(3, 1, b2s, b2t, b2u, r, a, apply_flags_z2_diag) + 
          get_Γ_comp(4, 1, b2s, b2t, b2u, r, a, apply_flags_z2_diag)

    # compute contributions for all lattice sites
    for j in eachindex(r.sites)
        val -= 2.0 * r.mult[j] * get_Γ_comp(4, j, b1s, b1t, b1u, r, a, apply_flags_z2_diag)
    end
...
function compute_dΣ_kernel_corr1(
    ...
    # compute local contributions
    val = get_Γ_comp(1, 1, b2s, b2t, b2u, r, da_Σ, apply_flags_z2_diag, ch_u = false) + 
          get_Γ_comp(2, 1, b2s, b2t, b2u, r, da_Σ, apply_flags_z2_diag, ch_u = false) + 
          get_Γ_comp(3, 1, b2s, b2t, b2u, r, da_Σ, apply_flags_z2_diag, ch_u = false) + 
          get_Γ_comp(4, 1, b2s, b2t, b2u, r, da_Σ, apply_flags_z2_diag, ch_u = false)

    # compute contributions for all lattice sites
    for j in eachindex(r.sites)
        val -= 2.0 * r.mult[j] * get_Γ_comp(4, j, b1s, b1t, b1u, r, da_Σ, apply_flags_z2_diag, ch_t = false)
    end
