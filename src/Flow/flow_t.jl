# compute the Katanin truncated flow equations in the t-channel for a frequency tuple (w1, w2, w3) and a given kernel on all lattice sites
function compute_channel_t_kat!(
    ...
    # compute integral
    ref = Λ + 0.5 * t
    val = max(2.0 * m.Ωt[end], m.νt[end], 5.0 * ref)
