...
function compute_dΣ!(
    ...
    @sync for i in 2 : length(m.σ)
        Threads.@spawn begin
            ...
            da.Σ[i]   = quadgk(integrand, -Inf, -4.0 * Λ, -2.0 * Λ, -Λ, 0.0, Λ, 2.0 * Λ, 4.0 * Λ, Inf, atol = Σ_tol[1], rtol = Σ_tol[2], order = 10)[1]
        end
    end
...
function compute_dΣ_corr!(
    ...
    @sync for i in 2 : length(m.σ)
        Threads.@spawn begin
            ...
            da_Σ.Σ[i] = quadgk(integrand, -Inf, -4.0 * Λ, -2.0 * Λ, -Λ, 0.0, Λ, 2.0 * Λ, 4.0 * Λ, Inf, atol = Σ_tol[1], rtol = Σ_tol[2], order = 10)[1]
        end
    end
