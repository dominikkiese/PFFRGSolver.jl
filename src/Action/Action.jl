...
# 2) if the maximum y_peak is located at x_peak > 0.0, ensure that a fraction p3 of the linearly spaced frequencies lies between 0.0 and x_peak (only if y_peak deviates by more than 15 percent from y(x = 0.0))
...
function scan(
    ...
    # check if the maximum y_peak is located at x_peak > 0.0 (only if y_peak deviates by more than 15 percent from y(x = 0.0))
    else 
