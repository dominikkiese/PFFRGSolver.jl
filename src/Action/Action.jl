...
# if the maximum is not located at the origin within tolerance 1 - p2, set linear extend via FWHM
...
function scan(
    ...
    # check that the maximum is located at the origin
    if abs(y[1]) / y_max >= 1.0 - p2
        ...
        while (p1 <= Δ <= p2) == false
            # if Δ is too large decrease the width by half a percent
            if Δ > p2
                δp *= 0.995
            # if Δ is too small increase the width by half a percent
            elseif Δ < p1
                δp *= 1.005
            end
        ...
        # parse result 
        δ = δp
    # if the maximum is not located at the origin, set linear extend via FWHM
    else
        # determine frequency corresponding to FWHM
        for i in y_idx : length(y)
            if abs(y[i]) / y_max < 0.5
                δp = min(x[i + 1], 0.1 * x[end])
                break 
            end 
        end 
...
function scan_channel(
    ...
    if maximum(abs.(q3_Ω)) > 1e-5
        scan_res = scan(Ω, q3_Ω, p_Ω[1], p_Ω[2], p_Ω[3], p_Ω[4] * Λ, p_Ω[5] * Λ, p_Ω[6], p_Ω[7] * Λ)
        Ω_lin    = scan_res[1]
        Ω_upper  = scan_res[2]
    end 
    ...
    if maximum(abs.(q3_ν_1)) > 1e-5
        scan_res = scan(ν, q3_ν_1, p_ν[1], p_ν[2], p_ν[3], p_ν[4] * Λ, p_ν[5] * Λ, p_ν[6], p_ν[7] * Λ)
        ν_lin    = scan_res[1]
        ν_upper  = scan_res[2]
    end 

    if maximum(abs.(q3_ν_2)) > 1e-5
        scan_res = scan(ν, q3_ν_2, p_ν[1], p_ν[2], p_ν[3], p_ν[4] * Λ, p_ν[5] * Λ, p_ν[6], p_ν[7] * Λ)
    ...
    if maximum(abs.(q3_ν_3)) > 1e-5
        scan_res = scan(ν, q3_ν_3, p_ν[1], p_ν[2], p_ν[3], p_ν[4] * Λ, p_ν[5] * Λ, p_ν[6], p_ν[7] * Λ)
...
function resample_from_to(
    ...
    for comp in eachindex(a_old.Γ)
        ...
        if res[1] > 0.0 
            Ωs_lins[comp]   = res[1]
            Ωs_uppers[comp] = res[2] 
        end 

        if res[3] > 0.0 
            νs_lins[comp]   = res[3]
            νs_uppers[comp] = res[4] 
        end 
    end
    ...
    for comp in eachindex(a_old.Γ)
        ...
        if res[1] > 0.0 
            Ωt_lins[comp]   = res[1]
            Ωt_uppers[comp] = res[2] 
        end 

        if res[3] > 0.0 
            νt_lins[comp]   = res[3]
            νt_uppers[comp] = res[4] 
        end 
    end
    ...
    for comp in eachindex(a_old.Γ)
        ...
        if res[1] > 0.0 
            Ωu_lins[comp]   = res[1]
            Ωu_uppers[comp] = res[2] 
        end 

        if res[3] > 0.0 
            νu_lins[comp]   = res[3]
            νu_uppers[comp] = res[4] 
        end 
    end

    # build new frequency meshes according to scanning results
    σ     = get_mesh(σ_lin, σ_upper, m_old.num_σ - 1, p_σ[1])
    Ωs    = Vector{Float64}[get_mesh(Ωs_lins[comp], Ωs_uppers[comp], m_old.num_Ω - 1, p_Ω[1]) for comp in eachindex(Ωs_lins)]
    νs    = Vector{Float64}[get_mesh(νs_lins[comp], νs_uppers[comp], m_old.num_ν - 1, p_ν[1]) for comp in eachindex(νs_lins)]
    Ωt    = Vector{Float64}[get_mesh(Ωt_lins[comp], Ωt_uppers[comp], m_old.num_Ω - 1, p_Ω[1]) for comp in eachindex(Ωt_lins)]
    νt    = Vector{Float64}[get_mesh(νt_lins[comp], νt_uppers[comp], m_old.num_ν - 1, p_ν[1]) for comp in eachindex(νt_lins)]
    Ωu    = Vector{Float64}[get_mesh(Ωu_lins[comp], Ωu_uppers[comp], m_old.num_Ω - 1, p_Ω[1]) for comp in eachindex(Ωu_lins)]
    νu    = Vector{Float64}[get_mesh(νu_lins[comp], νu_uppers[comp], m_old.num_ν - 1, p_ν[1]) for comp in eachindex(νu_lins)]
