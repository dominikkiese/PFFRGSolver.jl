...
function resample_from_to( 
    ...
    Ωs_lin = scan(m_old.Ωs, q3_Ω, 0.85, 1.5, 0.05 * Λ, 0.5 * Λ)
    νs_lin = scan(m_old.νs, q3_ν, 0.75, 1.5, 0.15 * Λ, 1.5 * Λ)  

    # scan the t channel
    q3     = a_old.Γ[max_comp].ch_t.q3
    q3_Ω   = q3[1, :, 1, 1]
    q3_ν   = Float64[q3[1, 1, x, x] .- q3[1, 1, end, end] for x in 1 : m_old.num_ν]
    Ωt_lin = scan(m_old.Ωt, q3_Ω, 0.85, 1.5, 0.05 * Λ, 0.5 * Λ)
    νt_lin = scan(m_old.νt, q3_ν, 0.75, 1.5, 0.15 * Λ, 1.5 * Λ) 
