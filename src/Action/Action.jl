...
# if the maximum is not located at the origin, set linear extend via position of the maximum
...
function scan(
    ...
    if max_idx > 1
        δ = 1.2 * x[max_idx]
    # else set linear extend such that p1 <= Δ <= p2, where Δ is the relative deviation between the value at the origin and the first finite frequency
    else
