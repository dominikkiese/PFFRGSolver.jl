...
#   a) the linear extent δ should be large enough to capture sharp peaks at finite frequencies, that is, δ >= FWHM of the peak
...
function scan(
    ...
    # a) check for peaks at finite frequencies and set δa as FWHM of the peak
    δa  = Inf
    idx = argmax(abs.(y))
    ...
    # b) check if the value at the origin is finite (i.e. in comparison to the maximum). If so, set δb such that p1 < Δ < p2
    δb = num_lin * x[2]

    if abs(y[1]) / abs(y[idx]) > 1e-1
        # compute Δ
