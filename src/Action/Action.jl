...
# returns upper bound such that the channel has decayed to a fraction p5 along the direction of the cut
# the upper bound must be at least as large as p6 times the new linear extent
function scan(
    x  :: Vector{Float64},
    y  :: Vector{Float64},
    p0 :: Float64,
