...
# if the maximum is strongly shifted from the origin, the linear extend is determined from the FWHM
...
function scan(
    ...
    if abs(y[1]) / y_max > 0.75
        ...
        # parse result 
        δ = δp
    # if the maximum is strongly shifted from the origin, set linear extend via FWHM
    else
...
function resample_from_to(
    ...
    # determine FWHM for self energy 
    σ_idx = argmax(abs.(a_old.Σ))
    σ_lin = m_old.σ[σ_idx]

    for i in σ_idx + 1 : m_old.num_σ
        if abs(a_old.Σ[i] / a_old.Σ[σ_idx]) < 0.5
            σ_lin = m_old.σ[i - 1]
            break 
        end 
    end
