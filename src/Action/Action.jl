...
# returns linear extend such that p1 <= Δ <= p2, where Δ is the relative deviation between the maximum at the origin and the value at the first finite frequency
# if the maximum is not located at the origin, set linear extent as p3 times the distance to the maximum
...
function resample_from_to(
    ...
    for i in eachindex(a_old.Γ)
        scan_res_s = scan_channel(Λ, p_Ω, p_ν, m_old.Ωs, m_old.νs, a_old.Γ[i].ch_s)
        scan_res_t = scan_channel(Λ, p_Ω, p_ν, m_old.Ωt, m_old.νt, a_old.Γ[i].ch_t)
        scan_res_u = scan_channel(Λ, p_Ω, p_ν, m_old.Ωu, m_old.νu, a_old.Γ[i].ch_u)
