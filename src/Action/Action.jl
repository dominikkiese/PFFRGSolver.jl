...
# if the maximum significantly stands out from the origin, set linear extent as p3 times position of the maximum
...
function scan(
    ...
    # set linear extent such that p1 <= Δ <= p2, where Δ is the relative deviation between the values at the origin and the first finite frequency
    if abs(y[1]) / max_val > 0.5
        # determine relative deviation between the values at the origin and the first finite frequency
...
function scan_channel(
    ...
    if maximum(abs.(q3_Ω)) > 1e-6
        Ω_lin = scan(Ω, q3_Ω, p_Ω[1], p_Ω[2], p_Ω[3], p_Ω[4], p_Ω[5] * Λ, p_Ω[6] * Λ)
    end 
    ...
    if maximum(abs.(q3_ν_1)) > 1e-6
        ν_lin_1 = scan(ν, q3_ν_1, p_ν[1], p_ν[2], p_ν[3], p_ν[4], p_ν[5] * Λ, p_ν[6] * Λ)
    end 

    if maximum(abs.(q3_ν_2)) > 1e-6
        ν_lin_2 = scan(ν, q3_ν_2, p_ν[1], p_ν[2], p_ν[3], p_ν[4], p_ν[5] * Λ, p_ν[6] * Λ)
    end 

    if maximum(abs.(q3_ν_3)) > 1e-6
        ν_lin_3 = scan(ν, q3_ν_3, p_ν[1], p_ν[2], p_ν[3], p_ν[4], p_ν[5] * Λ, p_ν[6] * Λ)
    end 
...
function resample_from_to(
    ...
    # scan the s channel
    Ωs_lin = zeros(Float64, length(m_old.Ωs))
    νs_lin = zeros(Float64, length(m_old.νs))
    ...
    # scan the t channel
    Ωt_lin = zeros(Float64, length(m_old.Ωt))
    νt_lin = zeros(Float64, length(m_old.νt))
    ...
    # scan the u channel
    Ωu_lin = zeros(Float64, length(m_old.Ωu))
    νu_lin = zeros(Float64, length(m_old.νu))
    ...
    σ     = get_mesh(σ_lin, 1000.0 * max(Λ, 0.5), m_old.num_σ - 1, p_σ[1])
    Ωs    = SVector(ntuple(comp -> get_mesh(Ωs_lin[comp], 750.0 * max(Λ, 0.5), m_old.num_Ω - 1, p_Ω[1]), length(Ωs_lin)))
    νs    = SVector(ntuple(comp -> get_mesh(νs_lin[comp], 500.0 * max(Λ, 0.5), m_old.num_ν - 1, p_ν[1]), length(νs_lin)))
    Ωt    = SVector(ntuple(comp -> get_mesh(Ωt_lin[comp], 750.0 * max(Λ, 0.5), m_old.num_Ω - 1, p_Ω[1]), length(Ωt_lin)))
    νt    = SVector(ntuple(comp -> get_mesh(νt_lin[comp], 500.0 * max(Λ, 0.5), m_old.num_ν - 1, p_ν[1]), length(νt_lin)))
    Ωu    = SVector(ntuple(comp -> get_mesh(Ωu_lin[comp], 750.0 * max(Λ, 0.5), m_old.num_Ω - 1, p_Ω[1]), length(Ωu_lin)))
    νu    = SVector(ntuple(comp -> get_mesh(νu_lin[comp], 500.0 * max(Λ, 0.5), m_old.num_ν - 1, p_ν[1]), length(νu_lin)))
