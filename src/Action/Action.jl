...
# the linear spacing is bounded from below by p3
# if the value at the origin vanishes within the integration tolerance, set linear extent to given reference value
function scan(
    x   :: Vector{Float64},
    y   :: Vector{Float64},
    p0  :: Float64,
...
function resample_from_to(
    ...
    νs_lin = Float64[12.0 * Λ for i in eachindex(a_old.Γ)]

    if Λ <= 2.0
        for comp in eachindex(a_old.Γ)
            Ωs_lin[comp] = scan(m_old.Ωs[comp], a_old.Γ[comp].ch_s.q3[1, :, 2, 2], p_Ω[1], p_Ω[2], p_Ω[3], p_Ω[4] * Λ, tol,  8.0 * Λ)
            νs_lin[comp] = scan(m_old.νs[comp], a_old.Γ[comp].ch_s.q3[1, 2, :, 2], p_ν[1], p_ν[2], p_ν[3], p_ν[4] * Λ, tol, 12.0 * Λ)
        end
    end

    # scan the t channel
    Ωt_lin = Float64[ 8.0 * Λ for i in eachindex(a_old.Γ)]
    νt_lin = Float64[12.0 * Λ for i in eachindex(a_old.Γ)]

    if Λ <= 2.0
        for comp in eachindex(a_old.Γ)
            Ωt_lin[comp] = scan(m_old.Ωt[comp], a_old.Γ[comp].ch_t.q3[1, :, 2, 2], p_Ω[1], p_Ω[2], p_Ω[3], p_Ω[4] * Λ, tol,  8.0 * Λ)
            νt_lin[comp] = scan(m_old.νt[comp], a_old.Γ[comp].ch_t.q3[1, 2, :, 2], p_ν[1], p_ν[2], p_ν[3], p_ν[4] * Λ, tol, 12.0 * Λ)
        end
    end
    ...
    ref   = max(Λ, 0.5)
    σ     = get_mesh(min(σ_lin, p_σ[1] * 1000.0 * ref), 1000.0 * ref, m_old.num_σ - 1, p_σ[1])
    Ωs    = SVector(ntuple(comp -> get_mesh(min(Ωs_lin[comp], p_Ω[1] * 400.0 * ref), 400.0 * ref, m_old.num_Ω - 1, p_Ω[1]), length(Ωs_lin)))
    νs    = SVector(ntuple(comp -> get_mesh(min(νs_lin[comp], p_ν[1] * 800.0 * ref), 800.0 * ref, m_old.num_ν - 1, p_ν[1]), length(νs_lin)))
    Ωt    = SVector(ntuple(comp -> get_mesh(min(Ωt_lin[comp], p_Ω[1] * 400.0 * ref), 400.0 * ref, m_old.num_Ω - 1, p_Ω[1]), length(Ωt_lin)))
    νt    = SVector(ntuple(comp -> get_mesh(min(νt_lin[comp], p_ν[1] * 800.0 * ref), 800.0 * ref, m_old.num_ν - 1, p_ν[1]), length(νt_lin)))
    Ωu    = SVector(ntuple(comp -> get_mesh(min(Ωu_lin[comp], p_Ω[1] * 400.0 * ref), 400.0 * ref, m_old.num_Ω - 1, p_Ω[1]), length(Ωu_lin)))
    νu    = SVector(ntuple(comp -> get_mesh(min(νu_lin[comp], p_ν[1] * 800.0 * ref), 800.0 * ref, m_old.num_ν - 1, p_ν[1]), length(νu_lin)))
