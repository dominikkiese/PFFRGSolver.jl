...
#   a) the linear extent δ should be large enough to capture sharp peaks at finite frequencies, that is, δ >= 'point where channel has decayed to 50 percent of peak value'.
...
function scan(
    ...
    # a) check for peaks at finite frequencies and set δa as 'point where channel has decayed to 50 percent of peak value'
    δa    = Inf
    idx   = 1
    ...
    if idx > 1 
        for i in idx + 1 : length(x)
            if abs(y[i] / y[idx]) <= 0.5
                # ensure, that there are not more than 25 percent of the linear frequencies placed below the peak position
                δa = max(x[i], x[idx] / max(1, floor(Int64, 0.25 * num_lin)) * num_lin)
                break
            end 
        end 
    end
